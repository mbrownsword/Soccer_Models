...
setwd("C:/Users/CORSAIR GAMING/Documents/FotMob Soccer Modelling/English League One")
...
ccode <- "ENG"
team_df <- data.frame(team_1 = c("Bolton Wanderers","Hartlepool United","Wolverhampton Wanderers","Brighton & Hove Albion","Newcastle United","Blackburn Rovers","Birmingham City","Wigan Athletic"),
                      team_2 = c("Bolton","Hartlepool","Wolverhampton","Brighton and Hove Albion","Newcastle","Blackburn","Birmingham","Wigan"))


fixtures <- fotmob_get_league_matches(l$ccode[leagues[1]],l$name[leagues[1]], cached = FALSE)
...
write.csv(e1_stats_s,paste0(ccode,"_",league_name,"_",gsub("-","",date[1],".csv")))
...
lineup_ratings <- player_stats_game[,c("team",colnames(player_stats_game)[which(grepl("_rating",colnames(player_stats_game)))])]



setwd("C:/Users/CORSAIR GAMING/Documents/FotMob Soccer Modelling/English League One")

source("running_eng_league_one_lines.R")
whsa
unique(c(unique(whsa$BARSTOOL_TEAM1),unique(whsa$BARSTOOL_TEAM2)))
unique(day_lineups$team)

team_df2 <-  data.frame(team_1 = c("Cheltenham" ),
                        team_2 = c("Cheltenham Town"   ))

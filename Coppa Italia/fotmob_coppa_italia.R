...
setwd("C:/Users/CORSAIR GAMING/Documents/FotMob Soccer Modelling/Italy Serie A")
...
league_name <- "Serie A"
ccode <- "ITA"
team_df <- data.frame(team_1 = c("Milan","Inter","Roma","Hellas Verona"),
                      team_2 = c("AC Milan" ,"Internazionale","AS Roma","Verona"))
...
game_data_by_team <- get_game_data_by_team(stat_game,pass_line_home,tkl_line_home,shots_line_home,sog_line_home,game_no, fixtures,
                                           ...
                                           sog_line_against_away,pass_line_away,tkl_line_away,shots_line_away,sog_line_away,f_teams_f, xg_line_home,
                                           xg_line_away,
                                           xgot_line_home,
                                           xgot_line_away,
                                           xa_line_home,
...
team_df2 <-  data.frame(team_1 = c("Hellas Verona" ,"Roma","Inter","Tottenham","Bournemouth"),
                        team_2 = c("Verona" ,"AS Roma","Internazionale","Tottenham Hotspur","AFC Bournemouth"))
...
source("running_serie_a_lines.R")
whsa
...
for(j in 1:length(fixtures$round)){
  ...
  game_end_check <- whsa$BARSTOOL_TEAM1_ML[whsa$BARSTOOL_TEAM1==fixtures$home$name[j]&whsa$BARSTOOL_TEAM2==fixtures$away$name[j]][1]
  ...
  print(paste0("Our modelled likelihood of ",fixtures$home$name[j]," winning this match OR DRAW is ",
               round((mean(spct_1)/sum(c(mean(spct_1),mean(wpct_X),mean(spct_2))))*100, 2)," percent, with a minimum chance of ",
               round(min(spct_1) * 100, 2),  " percent, and a maximum chance of ",round(max(spct_1) * 100, 2)," percent."))
  print(paste0("Our modelled likelihood of ",fixtures$away$name[j]," winning this match OR DRAW is ",
               round((mean(spct_2)/sum(c(mean(spct_1),mean(wpct_X),mean(spct_2))))*100, 2),
               " percent, with a minimum chance of ",
               round(min(spct_2) * 100, 2),  " percent, and a maximum chance of ",round(max(spct_2) * 100, 2)," percent."))
...
unique(all_player_tackles_lines$team)

b_team <- c("ACM"  ,"SAL" , "SAM" , "SASS","SPE" , "ATA" , "TNO" , "VER"  ,"LAZ" , "LEC" , "BOL" , "RMA" , "CRE" , "JUV" , "FIO" , "MON" , "INT",  "NAP" , "UDI" , "EMP")
f_team <- c(   "AC Milan" , "Salernitana"   , "Sampdoria",  "Sassuolo"   ,          "Spezia"      ,   "Atalanta"    ,   "Torino"    ,     "Verona"     ,         
             "Lazio"   , "Lecce"       ,   "Bologna" , "AS Roma"  ,      "Cremonese" ,    "Juventus"   ,           "Fiorentina"  ,   "Monza"      ,    "Internazionale", "Napoli",        
             "Udinese"      ,  "Empoli" )

team_df3 <- data.frame(b_team,f_team)
write.csv(team_df3,paste0("player_prop_teams",ccode,"_",league_name,"_",gsub("-","",date[1]),".csv"))
...
View(all_player_tackles_lines)
View(all_player_passes_lines)
View(all_player_shots_lines)
View(all_player_sogs_lines)
View(all_player_goals_lines)
View(all_player_ast_lines)
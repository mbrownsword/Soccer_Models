...
team_player_stats <- function(team1_squad,team1,team2,team){
  ...
  stat_afc <- team_player_data %>%
    group_by(page_url, position_row, is_home_team) %>%
  ...
  if(team==team1){
    stat_afc <- stat_afc[stat_afc$is_home_team==TRUE,]
  }else{
    stat_afc <- stat_afc[stat_afc$is_home_team==FALSE,]
  }
  
  stat_afc$is_home_team <- NULL
...
predicted_lineup <- function(team1_player_data, m_df, fixtures, team1, team2,team, team1_year_data){
  ...
  if(length(t$content$lineup)==1){
    ...
    if(team==team1){
      ...
      naplayers <- t$content$lineup$lineup[[2]]$nonAvailablePlayers[[1]]
      naplayers <- lapply(naplayers, function(x) { x["name"] <- NULL; x })
  ...
  if(length(na_players)==1){
    team1_data <- team1_year_data
  }else{
    if(length(which(team1_year_data$page_url_2 %in% na_players$pageUrl))==0){
      team1_data <- team1_year_data
    }else{
  ...
  for(a in (order(l_df$pos_row))){
    if(a==1){
      ...
      if(length(which(duplicated(players)))==0){
        
      }else{
        rating <- rating[-which(duplicated(players))]
        minutes <- minutes[-which(duplicated(players))]
        players <- players[-which(duplicated(players))]
      }
    ...
    team1_data <- team1_data[-which(team1_data$page_url_2  %in% unlist(starters[[a]]$page_url_2)),]
  }
  team1_lineup <- data.frame(rbindlist(starters))
  team1_subs <- team1_data[which(team1_data$position_row==0&team1_data$expected_goals_against==0),]
      ...
      if(length(t$content$lineup$lineup[[1]]$players)==0){
        ...
        if(length(t$content$lineup$bench$benchArr[[1]])==0){
          ...
          if(length(na_players_1$id)==0){
            ...
            team1_leftovers <- team1_player_data[-which(team1_player_data$page_url_2 %in% lineup_df$pageUrl),]
            if(length(which(team1_leftovers$page_url_2 %in% naplayers_1$pageUrl))==0){
              
            }else{
              team1_leftovers <- team1_leftovers[-which(team1_leftovers$page_url_2 %in% naplayers_1$pageUrl),]
            }
            team1_leftovers <- team1_leftovers[team1_leftovers$expected_goals_against==0,]
            team1_subs <- team1_leftovers[team1_leftovers$position_row==0,]
          }
          ...
          if(length(na_players_2$id)==0){
            ...
            team2_leftovers <- team2_player_data[-which(team2_player_data$page_url_2 %in% lineup_df_2$pageUrl),]
            if(length(which(team2_leftovers$page_url_2 %in% na_players_2$pageUrl))==0){
...
league_analysis <- function(leagues){
  ...
  if(leagues=="MEX Liga MX"){
    ...
    if(leagues=="USA MLS"){
      all_leagues[[1]] <- c("USA Major League Soccer")
    }else{
   ...
   if(length(all_leagues[[a]])>1){
    ...
    league_afc <- all_league_games %>%
      ...
      summarise(minutes = sum(stats_minutes_played, na.rm = T),
                ...
                avg_rating = mean(stats_fot_mob_rating[stats_fot_mob_rating!=0], na.rm = T),
                avg_elo = mean(team_rating[team_rating!=0&is_home_team==TRUE], na.rm = T),
                avg_opp_elo = mean(team_rating[team_rating!=0&is_home_team==FALSE], na.rm = T),
                avg_elo_dif = mean(team_rating[team_rating!=0&is_home_team==TRUE], na.rm = T) - mean(team_rating[team_rating!=0&is_home_team==FALSE], na.rm = T),
                avg_quality = mean(team_rating[team_rating!=0&is_home_team==TRUE], na.rm = T) + mean(team_rating[team_rating!=0&is_home_team==FALSE], na.rm = T)
                #avg_elo_dif = mean(elo_dif)
      )
    ...
    stat_game_xg <- stat_game[stat_game$minutes_xg>0,]
...
formation_analysis <- function(formation_matchup,positions1,positions2){
  ...
  formation_xg_games <- length(stat_game_position$expected_goals[!is.na(stat_game_position$expected_goals)])/(length(positions1)+length(positions2))
  formation_games <- length(stat_game_position$passes[!is.na(stat_game_position$passes)])/(length(positions1)+length(positions2))
...
get_game_player_data <- function(lineup_stats, formation_lines, fixtures_2, game_data, team1, team2){
  ...
  formation_xg_games<- formation_lines[[71]]
  formation_games<- formation_lines[[72]]
  ...
  fixtures_2$elo_dif_1 <- as.numeric(fixtures_2$spi1-fixtures_2$spi2)
  fixtures_2$elo_dif_2 <- as.numeric(fixtures_2$spi2-fixtures_2$spi1)
  for(h in 1:length(lineup_stats$player)){
    ...
    if(lineup_stats$positionRow[h]==0){
      ...
      if(lineup_stats$positionRow[h]==1){
        ...
        if(any(grepl(lineup_stats$team[h],fixtures_2$team1))){
          
          
          lineup_stats$shxpos[h] <- mean(c(shots_line_1_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shots_line_1_home$coefficients[[1]]))
         
          
          lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_1_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shotsongoal_line_1_home$coefficients[[1]]))
          
          
          
          lineup_stats$passxpos[h] <- mean(c(passes_line_1_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+passes_line_1_home$coefficients[[1]]))
          
          
          
          lineup_stats$tklxpos[h] <- mean(c(tackles_line_1_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+tackles_line_1_home$coefficients[[1]]))
         
          lineup_stats$xgxpos[h] <- mean(c(xg_line_1_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xg_line_1_home$coefficients[[1]]))
          
         
          
          lineup_stats$xgotxpos[h] <- mean(c(xgot_line_1_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xgot_line_1_home$coefficients[[1]]))
          
          
          
          lineup_stats$xaxpos[h] <- mean(c(xa_line_1_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xa_line_1_home$coefficients[[1]]))
          
          
          
        }else{
          
          lineup_stats$shxpos[h] <- mean(c(shots_line_1_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shots_line_1_away$coefficients[[1]]))
          
          lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_1_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shotsongoal_line_1_away$coefficients[[1]]))
          
          
          lineup_stats$passxpos[h] <- mean(c(passes_line_1_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+passes_line_1_away$coefficients[[1]]))
          
          
          lineup_stats$tklxpos[h] <- mean(c(tackles_line_1_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+tackles_line_1_away$coefficients[[1]]))
          
          lineup_stats$xgxpos[h] <- mean(c(xg_line_1_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xg_line_1_away$coefficients[[1]]))
          
          lineup_stats$xgotxpos[h] <- mean(c(xgot_line_1_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xgot_line_1_away$coefficients[[1]]))
          
          lineup_stats$xaxpos[h] <- mean(c(xa_line_1_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xa_line_1_away$coefficients[[1]]))
        ...
        if(lineup_stats$positionRow[h]==2){
          ...
          if(any(grepl(lineup_stats$team[h],fixtures_2$team1))){
            ...
            lineup_stats$shxpos[h] <- mean(c(shots_line_2_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shots_line_2_home$coefficients[[1]]))
            
            lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_2_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shotsongoal_line_2_home$coefficients[[1]]))
            
            
           lineup_stats$passxpos[h] <- mean(c(passes_line_2_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+passes_line_2_home$coefficients[[1]]))
            
            
            lineup_stats$tklxpos[h] <- mean(c(tackles_line_2_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+tackles_line_2_home$coefficients[[1]]))
            
            lineup_stats$xgxpos[h] <- mean(c(xg_line_2_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xg_line_2_home$coefficients[[1]]))
            
            lineup_stats$xgotxpos[h] <- mean(c(xgot_line_2_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xgot_line_2_home$coefficients[[1]]))
            
            lineup_stats$xaxpos[h] <- mean(c(xa_line_2_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xa_line_2_home$coefficients[[1]]))
            
            
            
          }else{
            
            lineup_stats$shxpos[h] <- mean(c(shots_line_2_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shots_line_2_away$coefficients[[1]]))
            
            lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_2_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shotsongoal_line_2_away$coefficients[[1]]))
            
            
            lineup_stats$passxpos[h] <- mean(c(passes_line_2_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+passes_line_2_away$coefficients[[1]]))
            
            
           lineup_stats$tklxpos[h] <- mean(c(tackles_line_2_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+tackles_line_2_away$coefficients[[1]]))
           lineup_stats$xgxpos[h] <- mean(c(xg_line_2_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xg_line_2_away$coefficients[[1]]))
            
           lineup_stats$xgotxpos[h] <- mean(c(xgot_line_2_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xgot_line_2_away$coefficients[[1]]))
             lineup_stats$xaxpos[h] <- mean(c(xa_line_2_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xa_line_2_away$coefficients[[1]]))
          ...
          if(lineup_stats$positionRow[h]==3){
            ...
            if(any(grepl(lineup_stats$team[h],fixtures_2$team1))){
              ...
              lineup_stats$shxpos[h] <- mean(c(shots_line_3_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shots_line_3_home$coefficients[[1]]))
             
              lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_3_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shotsongoal_line_3_home$coefficients[[1]]))
              
             lineup_stats$passxpos[h] <- mean(c(passes_line_3_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+passes_line_3_home$coefficients[[1]]))
              
             
              lineup_stats$tklxpos[h] <- mean(c(tackles_line_3_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+tackles_line_3_home$coefficients[[1]]))
             
              
              lineup_stats$xgxpos[h] <- mean(c(xg_line_3_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xg_line_3_home$coefficients[[1]]))
              
              
              lineup_stats$xgotxpos[h] <- mean(c(xgot_line_3_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xgot_line_3_home$coefficients[[1]]))
              
             
              lineup_stats$xaxpos[h] <- mean(c(xa_line_3_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xa_line_3_home$coefficients[[1]]))
              ...
              lineup_stats$shxpos[h] <- mean(c(shots_line_3_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shots_line_3_away$coefficients[[1]]))
              
              
              
              lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_3_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shotsongoal_line_3_away$coefficients[[1]]))
              
              
              
              
              lineup_stats$passxpos[h] <- mean(c(passes_line_3_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+passes_line_3_away$coefficients[[1]]))
              
             
              
              lineup_stats$tklxpos[h] <- mean(c(tackles_line_3_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+tackles_line_3_away$coefficients[[1]]))
              
              
             
              
              lineup_stats$xgxpos[h] <- mean(c(xg_line_3_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xg_line_3_away$coefficients[[1]]))
              
              
              
              lineup_stats$xgotxpos[h] <- mean(c(xgot_line_3_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xgot_line_3_away$coefficients[[1]]))
             
              
              lineup_stats$xaxpos[h] <- mean(c(xa_line_3_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xa_line_3_away$coefficients[[1]]))
            ...
            if(lineup_stats$positionRow[h]==4){
              ...
              if(any(grepl(lineup_stats$team[h],fixtures_2$team1))){
                ...
                lineup_stats$shxpos[h] <- mean(c(shots_line_4_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shots_line_4_home$coefficients[[1]]))
                
               
                
                lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_4_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shotsongoal_line_4_home$coefficients[[1]]))
                
                
               
                
                lineup_stats$passxpos[h] <- mean(c(passes_line_4_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+passes_line_4_home$coefficients[[1]]))
                
                
               
                
                lineup_stats$tklxpos[h] <- mean(c(tackles_line_4_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+tackles_line_4_home$coefficients[[1]]))
                
                
                if(is.na(xg_line_4_home)){
                  lineup_stats$xgxpos[h] <- mean(c(xg_line_5_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xg_line_5_home$coefficients[[1]]))
                  
                  
                  
                  lineup_stats$xgotxpos[h] <- mean(c(xgot_line_5_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xgot_line_5_home$coefficients[[1]]))
                  
                  
                  
                  lineup_stats$xaxpos[h] <- mean(c(xa_line_5_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xa_line_5_home$coefficients[[1]]))
                  
                }else{
                  lineup_stats$xgxpos[h] <- mean(c(xg_line_4_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xg_line_4_home$coefficients[[1]]))
                  
                  
                  
                  lineup_stats$xgotxpos[h] <- mean(c(xgot_line_4_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xgot_line_4_home$coefficients[[1]]))
                  
                  
                  
                  lineup_stats$xaxpos[h] <- mean(c(xa_line_4_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+xa_line_4_home$coefficients[[1]]))
                ...
                lineup_stats$shxpos[h] <- mean(c(shots_line_4_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shots_line_4_away$coefficients[[1]]))
               
                
                lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_4_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shotsongoal_line_4_away$coefficients[[1]]))
                
               
                
                lineup_stats$passxpos[h] <- mean(c(passes_line_4_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+passes_line_4_away$coefficients[[1]]))
                
               
                
                lineup_stats$tklxpos[h] <- mean(c(tackles_line_4_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+tackles_line_4_away$coefficients[[1]]))
                
                
               
                
                lineup_stats$xgxpos[h] <- mean(c(xg_line_4_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xg_line_4_away$coefficients[[1]]))
               
                
                lineup_stats$xgotxpos[h] <- mean(c(xgot_line_4_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xgot_line_4_away$coefficients[[1]]))
               
                
                lineup_stats$xaxpos[h] <- mean(c(xa_line_4_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xa_line_4_away$coefficients[[1]]))
              ...
              if(lineup_stats$positionRow[h]==5){
                ...
                if(any(grepl(lineup_stats$team[h],fixtures_2$team1))){
                  ...
                  lineup_stats$shxpos[h] <- mean(c(shots_line_5_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shots_line_5_home$coefficients[[1]]))
                  
                 
                  lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_5_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+shotsongoal_line_5_home$coefficients[[1]]))
                  
                  
                  
                  lineup_stats$passxpos[h] <- mean(c(passes_line_5_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+passes_line_5_home$coefficients[[1]]))
                  
                 
                  
                  lineup_stats$tklxpos[h] <- mean(c(tackles_line_5_home$coefficients[[2]]*fixtures_2$elo_dif_1[which(fixtures_2$team1==lineup_stats$team[h])]+tackles_line_5_home$coefficients[[1]]))
                  ...
                  lineup_stats$shxpos[h] <- mean(c(shots_line_5_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shots_line_5_away$coefficients[[1]]))
                 
                  
                  lineup_stats$sogxpos[h] <- mean(c(shotsongoal_line_5_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+shotsongoal_line_5_away$coefficients[[1]]))
                  
                 
                  
                  lineup_stats$passxpos[h] <- mean(c(passes_line_5_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+passes_line_5_away$coefficients[[1]]))
                
                  lineup_stats$tklxpos[h] <- mean(c(tackles_line_5_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+tackles_line_5_away$coefficients[[1]]))
                  
                  
                  
                  lineup_stats$xgxpos[h] <- mean(c(xg_line_5_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xg_line_5_away$coefficients[[1]]))
                  
                 
                  
                  lineup_stats$xgotxpos[h] <- mean(c(xgot_line_5_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xgot_line_5_away$coefficients[[1]]))
                  
                  
                  
                  lineup_stats$xaxpos[h] <- mean(c(xa_line_5_away$coefficients[[2]]*fixtures_2$elo_dif_2[which(fixtures_2$team2==lineup_stats$team[h])]+xa_line_5_away$coefficients[[1]]))
  ...
  for(q in 1:length(lineup_stats$pageUrl)){
    lineup_stats$passxpos[q] <- ifelse(formation_games>=50,(lineup_stats$passxpos[q]+lineup_stats$passxpos2[q])/2,lineup_stats$passxpos2[q])
    lineup_stats$tklxpos[q] <- ifelse(formation_games>=50,(lineup_stats$tklxpos[q]+lineup_stats$tklxpos2[q])/2,lineup_stats$tklxpos2[q])
    lineup_stats$shxpos[q] <- ifelse(formation_games>=50,(lineup_stats$shxpos[q]+lineup_stats$shxpos2[q])/2,lineup_stats$shxpos2[q])
    lineup_stats$sogxpos[q] <- ifelse(formation_games>=50,(lineup_stats$sogxpos[q]+lineup_stats$sogxpos2[q])/2,lineup_stats$sogxpos2[q])
    lineup_stats$xgxpos[q] <- ifelse(formation_xg_games>=50,(lineup_stats$xgxpos[q]+lineup_stats$xgxpos2[q])/2,lineup_stats$xgxpos2[q])
    lineup_stats$xgotxpos[q] <- ifelse(formation_xg_games>=50,(lineup_stats$xgotxpos[q]+lineup_stats$xgotxpos2[q])/2,lineup_stats$xgotxpos2[q])
    lineup_stats$xaxpos[q] <- ifelse(formation_xg_games>=50,(lineup_stats$xaxpos[q]+lineup_stats$xaxpos2[q])/2,lineup_stats$xaxpos2[q])
  ...
  if(FALSE==FALSE){
    ...
    team1_stats <- player_stats_game[player_stats_game$team==team1,]
    team2_stats<- player_stats_game[player_stats_game$team==team2,][seq(dim(player_stats_game[player_stats_game$team==team2,])[1],1),]
    team1_stats <- team1_stats[order(-team1_stats$expected_xg),]
    team2_stats <- team2_stats[order(-team2_stats$expected_xg),]
    no1s <- which(team1_stats$minutes>=180)
    no2s <- which(team2_stats$minutes>=180)
    no1sxg <- which(team1_stats$minutes_xg>=180)
    no2sxg <- which(team2_stats$minutes_xg>=180)
    
    
    
    p19 <- sum(team1_stats$pass_90min[intersect(no1s,no2s)])/sum(c(team1_stats$pass_90min[intersect(no1s,no2s)],
                                                                   team2_stats$pass_90min[intersect(no1s,no2s)]))
    t19 <- sum(team1_stats$tkl_90min[intersect(no1s,no2s)])/sum(c(team1_stats$tkl_90min[intersect(no1s,no2s)],
                                                                  team2_stats$tkl_90min[intersect(no1s,no2s)]))
    
    p29 <- sum(team2_stats$pass_90min[intersect(no1s,no2s)])/sum(c(team1_stats$pass_90min[intersect(no1s,no2s)],
                                                                   team2_stats$pass_90min[intersect(no1s,no2s)]))
    t29 <- sum(team2_stats$tkl_90min[intersect(no1s,no2s)])/sum(c(team1_stats$tkl_90min[intersect(no1s,no2s)],
                                                                  team2_stats$tkl_90min[intersect(no1s,no2s)]))
    
    team1_stats <- team1_stats[order(-team1_stats$expected_shots),]
    team2_stats <- team2_stats[order(-team2_stats$expected_shots),]
    no1s <- which(team1_stats$minutes>=180)
    no2s <- which(team2_stats$minutes>=180)
    no1sxg <- which(team1_stats$minutes_xg>=180)
    no2sxg <- which(team2_stats$minutes_xg>=180)
    
    sh19 <- sum(team1_stats$shots_90min[intersect(no1s,no2s)])/sum(c(team1_stats$shots_90min[intersect(no1s,no2s)],
                                                                     team2_stats$shots_90min[intersect(no1s,no2s)]))
    
    sh29 <- sum(team2_stats$shots_90min[intersect(no1s,no2s)])/sum(c(team1_stats$shots_90min[intersect(no1s,no2s)],
                                                                     team2_stats$shots_90min[intersect(no1s,no2s)]))
    
    team1_stats <- team1_stats[order(-team1_stats$expected_sogs),]
    team2_stats <- team2_stats[order(-team2_stats$expected_sogs),]
    no1s <- which(team1_stats$minutes>=180)
    no2s <- which(team2_stats$minutes>=180)
    no1sxg <- which(team1_stats$minutes_xg>=180)
    no2sxg <- which(team2_stats$minutes_xg>=180)
    ...
    sog19 <- sum(team1_stats$sog_90min[intersect(no1s,no2s)])/sum(c(team1_stats$sog_90min[intersect(no1s,no2s)],
                                                                    team2_stats$sog_90min[intersect(no1s,no2s)]))
    sog29 <- sum(team2_stats$sog_90min[intersect(no1s,no2s)])/sum(c(team1_stats$sog_90min[intersect(no1s,no2s)],
                                                                    team2_stats$sog_90min[intersect(no1s,no2s)]))
    ...
    xg19 <- sum(team1_stats$xg_90min[intersect(no1sxg,no2sxg)])/sum(c(team1_stats$xg_90min[intersect(no1sxg,no2sxg)],
                                                                      team2_stats$xg_90min[intersect(no1sxg,no2sxg)]))
    
    xg29 <- sum(team2_stats$xg_90min[intersect(no1sxg,no2sxg)])/sum(c(team1_stats$xg_90min[intersect(no1sxg,no2sxg)],
                                                                      team2_stats$xg_90min[intersect(no1sxg,no2sxg)]))
    
    team1_stats <- team1_stats[order(-team1_stats$expected_xgot),]
    team2_stats <- team2_stats[order(-team2_stats$expected_xgot),]
    no1s <- which(team1_stats$minutes>=180)
    no2s <- which(team2_stats$minutes>=180)
    no1sxg <- which(team1_stats$minutes_xg>=180)
    no2sxg <- which(team2_stats$minutes_xg>=180)
    ...
    xgot19 <- sum(team1_stats$xgot_90min[intersect(no1sxg,no2sxg)])/sum(c(team1_stats$xgot_90min[intersect(no1sxg,no2sxg)],
                                                                          team2_stats$xgot_90min[intersect(no1sxg,no2sxg)]))
    
    xgot29 <- sum(team2_stats$xgot_90min[intersect(no1sxg,no2sxg)])/sum(c(team1_stats$xgot_90min[intersect(no1sxg,no2sxg)],
                                                                          team2_stats$xgot_90min[intersect(no1sxg,no2sxg)]))
    
    team1_stats <- team1_stats[order(-team1_stats$expected_xa),]
    team2_stats <- team2_stats[order(-team2_stats$expected_xa),]
    no1s <- which(team1_stats$minutes>=180)
    no2s <- which(team2_stats$minutes>=180)
    no1sxg <- which(team1_stats$minutes_xg>=180)
    no2sxg <- which(team2_stats$minutes_xg>=180)
    ...
    xa19 <- sum(team1_stats$xa_90min[intersect(no1sxg,no2sxg)])/sum(c(team1_stats$xa_90min[intersect(no1sxg,no2sxg)],
                                                                      team2_stats$xa_90min[intersect(no1sxg,no2sxg)]))
    
    xa29 <- sum(team2_stats$xa_90min[intersect(no1sxg,no2sxg)])/sum(c(team1_stats$xa_90min[intersect(no1sxg,no2sxg)],
                                                                      team2_stats$xa_90min[intersect(no1sxg,no2sxg)]))
    ...
    p1e <- sum(player_stats_game$expected_passes[player_stats_game$team==team1], na.rm = T)/sum(player_stats_game$expected_passes, na.rm = T)
    t1e <- sum(player_stats_game$expected_tackles[player_stats_game$team==team1], na.rm = T)/sum(player_stats_game$expected_tackles, na.rm = T)
    sh1e <- sum(player_stats_game$expected_shots[player_stats_game$team==team1], na.rm = T)/sum(player_stats_game$expected_shots, na.rm = T)
    sog1e <- sum(player_stats_game$expected_sogs[player_stats_game$team==team1], na.rm = T)/sum(player_stats_game$expected_sogs, na.rm = T)
    xg1e <- sum(player_stats_game$expected_xg[player_stats_game$team==team1], na.rm = T)/sum(player_stats_game$expected_xg, na.rm = T)
    xgot1e <- sum(player_stats_game$expected_xgot[player_stats_game$team==team1], na.rm = T)/sum(player_stats_game$expected_xgot, na.rm = T)
    xa1e <- sum(player_stats_game$expected_xa[player_stats_game$team==team1], na.rm = T)/sum(player_stats_game$expected_xa, na.rm = T)
    
    p2e <- sum(player_stats_game$expected_passes[player_stats_game$team==team2], na.rm = T)/sum(player_stats_game$expected_passes, na.rm = T)
    t2e <- sum(player_stats_game$expected_tackles[player_stats_game$team==team2], na.rm = T)/sum(player_stats_game$expected_tackles, na.rm = T)
    sh2e <- sum(player_stats_game$expected_shots[player_stats_game$team==team2], na.rm = T)/sum(player_stats_game$expected_shots, na.rm = T)
    sog2e <- sum(player_stats_game$expected_sogs[player_stats_game$team==team2], na.rm = T)/sum(player_stats_game$expected_sogs, na.rm = T)
    xg2e <- sum(player_stats_game$expected_xg[player_stats_game$team==team2], na.rm = T)/sum(player_stats_game$expected_xg, na.rm = T)
    xgot2e <- sum(player_stats_game$expected_xgot[player_stats_game$team==team2], na.rm = T)/sum(player_stats_game$expected_xgot, na.rm = T)
    xa2e <- sum(player_stats_game$expected_xa[player_stats_game$team==team2], na.rm = T)/sum(player_stats_game$expected_xa, na.rm = T)
    
    
    
    if(length(intersect(no1sxg,no2sxg))>=10){
      
      p1_share <- ((2*p19)+p1e)/3
      t1_share <- ((2*t19)+t1e)/3
      sh1_share <- ((2*sh19)+sh1e)/3
      sog1_share <- ((2*sog19)+sog1e)/3
      xg1_share <- ((2*xg19)+xg1e)/3
      xgot1_share <- ((2*xgot19)+xgot1e)/3
      xa1_share <- ((2*xa19)+xa1e)/3
      
      p2_share <- ((2*p29)+p2e)/3
      t2_share <- ((2*t29)+t2e)/3
      sh2_share <- ((2*sh29)+sh2e)/3
      sog2_share <- ((2*sog29)+sog2e)/3
      xg2_share <- ((2*xg29)+xg2e)/3
      xgot2_share <- ((2*xgot29)+xgot2e)/3
      xa2_share <- ((2*xa29)+xa2e)/3
      
      xgot1_share <- mean(c(xgot1_share,xg1_share))
      xgot2_share <- mean(c(xgot2_share,xg2_share))
      
      xgot1e <- mean(c(xgot1e,xg1e))
      xgot2e <- mean(c(xgot2e,xg2e))
      
      player_stats_game$expected_passes[player_stats_game$team==team1] <- player_stats_game$expected_passes[player_stats_game$team==team1]*mean(c(xa1_share/xa1e,xgot1_share/xgot1e))
      player_stats_game$expected_tackles[player_stats_game$team==team1] <- player_stats_game$expected_tackles[player_stats_game$team==team1]*mean(c(xa1_share/xa1e,xgot1_share/xgot1e))
      player_stats_game$expected_shots[player_stats_game$team==team1] <- player_stats_game$expected_shots[player_stats_game$team==team1]*mean(c(xa1_share/xa1e,xgot1_share/xgot1e))
      player_stats_game$expected_sogs[player_stats_game$team==team1] <- player_stats_game$expected_sogs[player_stats_game$team==team1]*mean(c(xa1_share/xa1e,xgot1_share/xgot1e))
      player_stats_game$expected_xg[player_stats_game$team==team1] <- player_stats_game$expected_xg[player_stats_game$team==team1]*mean(c(xa1_share/xa1e,xgot1_share/xgot1e))
      player_stats_game$expected_xgot[player_stats_game$team==team1] <- player_stats_game$expected_xgot[player_stats_game$team==team1]*mean(c(xa1_share/xa1e,xgot1_share/xgot1e))
      player_stats_game$expected_xa[player_stats_game$team==team1] <- player_stats_game$expected_xa[player_stats_game$team==team1]*mean(c(xa1_share/xa1e,xgot1_share/xgot1e))
      
      player_stats_game$expected_passes[player_stats_game$team==team2] <- player_stats_game$expected_passes[player_stats_game$team==team2]*mean(c(xa2_share/xa2e,xgot2_share/xgot2e))
      player_stats_game$expected_tackles[player_stats_game$team==team2] <- player_stats_game$expected_tackles[player_stats_game$team==team2]*mean(c(xa2_share/xa2e,xgot2_share/xgot2e))
      player_stats_game$expected_shots[player_stats_game$team==team2] <- player_stats_game$expected_shots[player_stats_game$team==team2]*mean(c(xa2_share/xa2e,xgot2_share/xgot2e))
      player_stats_game$expected_sogs[player_stats_game$team==team2] <- player_stats_game$expected_sogs[player_stats_game$team==team2]*mean(c(xa2_share/xa2e,xgot2_share/xgot2e))
      player_stats_game$expected_xg[player_stats_game$team==team2] <- player_stats_game$expected_xg[player_stats_game$team==team2]*mean(c(xa2_share/xa2e,xgot2_share/xgot2e))
      player_stats_game$expected_xgot[player_stats_game$team==team2] <- player_stats_game$expected_xgot[player_stats_game$team==team2]*mean(c(xa2_share/xa2e,xgot2_share/xgot2e))
      player_stats_game$expected_xa[player_stats_game$team==team2] <- player_stats_game$expected_xa[player_stats_game$team==team2]*mean(c(xa2_share/xa2e,xgot2_share/xgot2e))
  ...
  player_stats_game$expected_xgot <- player_stats_game$expected_xgot*player_stats_game$exp_min/90
...
game_predict <- function(league_lines, fixtures, lines, player_stats_game, league, goalie_ratings, date){
  stat_game <- league_lines[[1]][[1]]
#  sdspi <- sd(stat_game$avg_elo)
#  data <- data.frame(xgot = stat_game$expected_goals[stat_game$is_home_team==TRUE&fixtures$spi1<=(stat_game$avg_elo+sdspi)&fixtures$spi1>=(stat_game$avg_elo-sdspi)],
   #                  goals = stat_game$goals[stat_game$is_home_team==TRUE&fixtures$spi1<=(stat_game$avg_elo+sdspi)&fixtures$spi1>=(stat_game$avg_elo-sdspi)])
  
  # Fit a Poisson regression model
#  model_h <- glm(goals ~ xgot, data = data, family = "poisson")
  
  
 # data <- data.frame(xgot = stat_game$expected_goals[stat_game$is_home_team==FALSE&fixtures$spi2<=(stat_game$avg_elo+sdspi)&fixtures$spi2>=(stat_game$avg_elo-sdspi)],
  #                   goals = stat_game$goals[stat_game$is_home_team==FALSE&fixtures$spi2<=(stat_game$avg_elo+sdspi)&fixtures$spi2>=(stat_game$avg_elo-sdspi)])
  
  # Fit a Poisson regression model
#  model_a <- glm(goals ~ xgot, data = data, family = "poisson")
  ...
  h_conv_rate <- (stat_game$expected_goals_ot[stat_game$expected_ast!=0&stat_game$is_home_team==TRUE]-stat_game$expected_ast[stat_game$expected_ast!=0&stat_game$is_home_team==TRUE])
  a_conv_rate <- (stat_game$expected_goals_ot[stat_game$expected_ast!=0&stat_game$is_home_team==FALSE]-stat_game$expected_ast[stat_game$expected_ast!=0&stat_game$is_home_team==FALSE])
  
  conv_rate_h <- lm(h_conv_rate~stat_game$avg_elo_dif[stat_game$expected_ast!=0&stat_game$is_home_team==TRUE])
  conv_rate_a <- lm(a_conv_rate~stat_game$avg_elo_dif[stat_game$expected_ast!=0&stat_game$is_home_team==FALSE])
  
  h_conv_rate <- (stat_game$expected_goals_ot[stat_game$expected_goals!=0&stat_game$is_home_team==TRUE]-stat_game$expected_goals[stat_game$expected_goals!=0&stat_game$is_home_team==TRUE])
  a_conv_rate <- (stat_game$expected_goals_ot[stat_game$expected_goals!=0&stat_game$is_home_team==FALSE]-stat_game$expected_goals[stat_game$expected_goals!=0&stat_game$is_home_team==FALSE])
  
  conv_rate_hxg <- lm(h_conv_rate~stat_game$avg_elo_dif[stat_game$expected_goals!=0&stat_game$is_home_team==TRUE])
  conv_rate_axg <- lm(a_conv_rate~stat_game$avg_elo_dif[stat_game$expected_goals!=0&stat_game$is_home_team==FALSE])
  ...
  for(j in 1:length(fixtures$round)){
    ...
    sink(iconv(paste0(gsub("\\.","",paste0(gsub("-","",date)," ",paste0(l$ccode[l$id==id[zz]]," ",gsub("\\.","",l$name[l$id==id[zz]])),"_",team1," v ",team2,"_card")),".txt"),"latin1","ascii",sub = ""), split = TRUE)
   ...
   # }
    ...
    #XG Model
    h_conv_rate <- as.numeric(conv_rate_hxg$coefficients[2]*(fixtures$spi1-fixtures$spi2) + conv_rate_hxg$coefficients[1])
    a_conv_rate <- as.numeric(conv_rate_axg$coefficients[2]*(fixtures$spi2-fixtures$spi1) + conv_rate_axg$coefficients[1])
    
    h_score <- sum(player_stats_game$expected_xg[player_stats_game$team==fixtures$home$name[j]&player_stats_game$match_id==fixtures$id[j]])+h_conv_rate
    a_score <- sum(player_stats_game$expected_xg[player_stats_game$team==fixtures$away$name[j]&player_stats_game$match_id==fixtures$id[j]])+a_conv_rate
    ...
    for (i in 1:n_sims) {
      ...
      if ((team_one_goals + team_two_goals)>=3) {
        overs <- overs + 1
      } else {
        unders <- unders + 1
      }
    ...
    h_share_1 <- h_score/(h_score+a_score)
    a_share_1 <- a_score/(h_score+a_score)
    ...
    h_conv_rate <- as.numeric(conv_rate_h$coefficients[2]*(fixtures$spi1-fixtures$spi2) + conv_rate_h$coefficients[1])
    a_conv_rate <- as.numeric(conv_rate_a$coefficients[2]*(fixtures$spi2-fixtures$spi1) + conv_rate_a$coefficients[1])
...
send_to_git <- function(league,team1,team2){
  ...
  file_path <- paste0(getwd(),"/",iconv(paste0(gsub("\\.","",paste0(gsub("-","",date)," ",paste0(l$ccode[l$id==id[zz]]," ",gsub("\\.","",l$name[l$id==id[zz]])),"_",team1," v ",team2,"_card")),".txt"),"latin1","ascii",sub = ""))
  file_name <- iconv(paste0(gsub("\\.","",paste0(gsub("-","",date)," ",paste0(l$ccode[l$id==id[zz]]," ",gsub("\\.","",l$name[l$id==id[zz]])),"_",team1," v ",team2,"_card")),".txt"),"latin1","ascii",sub = "")
...
sub_adjustments <- function(team1_subs,team2_subs,player_stats_game, team1,team2){
  ...
  if(length(which(is.na(subs1$minutes)))==0){
   
  }else{
    subs2 <- subs2[-which(is.na(subs1$minutes)),]
    subs1 <- subs1[-which(is.na(subs1$minutes)),]
  }
  ...
  hsogr <- sum(player_stats_game$expected_sogs[player_stats_game$team==team1])/sum(player_stats_game$expected_sogs)
  hshr <-  sum(player_stats_game$expected_shots[player_stats_game$team==team1])/sum(player_stats_game$expected_shots)
  hxgr <-  sum(player_stats_game$expected_xg[player_stats_game$team==team1])/sum(player_stats_game$expected_xg)
  hxgotr <-  sum(player_stats_game$expected_xgot[player_stats_game$team==team1])/sum(player_stats_game$expected_xgot)
  hxar <-  sum(player_stats_game$expected_xa[player_stats_game$team==team1])/sum(player_stats_game$expected_xa)
  
  asogr <- sum(player_stats_game$expected_sogs[player_stats_game$team==team2])/sum(player_stats_game$expected_sogs)
  ashr <-  sum(player_stats_game$expected_shots[player_stats_game$team==team2])/sum(player_stats_game$expected_shots)
  axgr <-  sum(player_stats_game$expected_xg[player_stats_game$team==team2])/sum(player_stats_game$expected_xg)
  axgotr <- sum(player_stats_game$expected_xgot[player_stats_game$team==team2])/sum(player_stats_game$expected_xgot)
  axar <-  sum(player_stats_game$expected_xa[player_stats_game$team==team2])/sum(player_stats_game$expected_xa)
  
  tsubs1 <- subs1
  tsubs2 <- subs2
  
  subs1$expected_sogs <- subs1$expected_sogs*(hsogr/(sum(tsubs1$expected_sogs)/sum(c(tsubs1$expected_sogs,
                                                                                     tsubs2$expected_sogs))))
  subs1$expected_shots <- subs1$expected_shots*(hshr/(sum(tsubs1$expected_shots)/sum(c(tsubs1$expected_shots,
                                                                                       tsubs2$expected_shots))))
  subs1$expected_xg <- subs1$expected_xg*(hxgr/(sum(tsubs1$expected_xg)/sum(c(tsubs1$expected_xg,
                                                                              tsubs2$expected_xg))))
  subs1$expected_xgot <- subs1$expected_xgot*(hxgotr/(sum(tsubs1$expected_xgot)/sum(c(tsubs1$expected_xgot,
                                                                                      tsubs2$expected_xgot))))
  subs1$expected_xa <- subs1$expected_xa*(hxar/(sum(tsubs1$expected_xa)/sum(c(tsubs1$expected_xa,
                                                                              tsubs2$expected_xa))))
  
  subs2$expected_sogs <- subs2$expected_sogs*(asogr/(sum(tsubs2$expected_sogs)/sum(c(tsubs2$expected_sogs,
                                                                                     tsubs1$expected_sogs))))
  subs2$expected_shots <- subs2$expected_shots*(ashr/(sum(tsubs2$expected_shots)/sum(c(tsubs2$expected_shots,
                                                                                       tsubs1$expected_shots))))
  subs2$expected_xg <- subs2$expected_xg*(axgr/(sum(tsubs2$expected_xg)/sum(c(tsubs2$expected_xg,
                                                                              tsubs1$expected_xg))))
  subs2$expected_xgot <- subs2$expected_xgot*(axgotr/(sum(tsubs2$expected_xgot)/sum(c(tsubs2$expected_xgot,
                                                                                      tsubs1$expected_xgot))))
  subs2$expected_xa <- subs2$expected_xa*(axar/(sum(tsubs2$expected_xa)/sum(c(tsubs2$expected_xa,
                                                                              tsubs1$expected_xa))))

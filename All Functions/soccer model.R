...
leagues_df[length(leagues_df$mattid)+1,] <- ""
leagues_df$b_code_1[length(leagues_df$mattid)] <- "usa"
leagues_df$b_code_2[length(leagues_df$mattid)] <- "mls"
...
for(zz in 1:length(id)){
  ...
  for(yy in 1:length(fixtures$round)){
    ...
    team1_player_data <- team_player_stats(team1_squad, team1,team2,team1)
    ...
    team2_squad$id[team2_squad$name=="Mark Robinson"] <- "25269"
    team2_squad$name[team2_squad$name=="Mark Robinson"] <- "Mark Robins"
    ...
    team2_player_data <- team_player_stats(team2_squad, team1,team2,team2)
    ...
    if(league!="MEX Liga MX" & league !="USA MLS"){
      setwd(paste0(paste0(wspc,"Fotmob Soccer Modelling/All League Data")))
      league_files <- list.files()
    ...
    if(length(t$content$lineup)==0){
      ...
      if(length(t$content$lineup)==1){
        ...
        if(length(t$content$lineup$lineup[[1]]$players)==0){
          ...
          if(special==T){
            lineup_stats$positionRow[lineup_stats$pageUrl2=="/players/391671/dogan-erdogan"] <- 3
            lineup_stats$positionRow[lineup_stats$pageUrl2=="/players/645576/kristijan-bistrovic"] <- 3
          }
            ...
            for(i in 1:length(lineup_stats$pageUrl)){
              if(lineup_stats$minutes[i]==0){
                ...
                if(length(replace_data$page_url_2)==0){
                  ...
                  if(length(which(is.na(replace_data$avg_rating)))==0){
                    
                  }else{
                    replace_data <- replace_data[-which(is.na(replace_data$avg_rating)),]
                  }
            ...
            for(i in 1:length(lineup_stats$pageUrl)){
              if(is.na(lineup_stats$pass_90min[i])){
                ...
                if(length(replace_data$page_url_2)==0){
                  ...
                  replace_data <- replace_data[replace_data$avg_rating==min(replace_data$avg_rating[replace_data$minutes_xg>180], na.rm = T),]
                  lineup_stats[i,c(col_select[2:length(col_select)])] <- replace_data[1,c(2:27)]
                  lineup_stats[i,c(col_select[1])] <- lineup_stats$pageUrl2[i]
                  next()
                }
    ...
    formation_lines <- formation_analysis(formation_matchup,positions1,positions2)
    ...
    pr1 <- mean(h_lr,h_pr)
    sh1 <- mean(h_lr,h_shr)
    sog1 <- mean(h_lr,h_sogr)
    xg1 <- mean(h_lr,h_xgr)
    xgot1 <- mean(h_lr,h_xgotr)
    xa1 <- mean(h_lr,h_xar)
    
    pr2 <- mean(a_lr,a_pr)
    sh2 <- mean(a_lr,a_shr)
    sog2 <- mean(a_lr,a_sogr)
    xg2 <- mean(a_lr,a_xgr)
    xgot2 <- mean(a_lr,a_xgotr)
    xa2 <- mean(a_lr,a_xar)
    ...
    game_data$passes_1 <- game_data$passes_1*pr1*m_pr[1]
    game_data$passes_2 <- game_data$passes_2*pr2*m_pr[2]
    game_data$team1_shots <- game_data$team1_shots*sh1*m_shr[1]
    game_data$team2_shots <- game_data$team2_shots*sh2*m_shr[2]
    game_data$team1_sogs <- game_data$team1_sogs*sog1*m_sogr[1]
    game_data$team2_sogs <- game_data$team2_sogs*sog2*m_sogr[2]
    game_data$team1_xg <- game_data$team1_xg*xg1*m_xgr[1]
    game_data$team2_xg <- game_data$team2_xg*xg2*m_xgr[2]
    game_data$team1_xgot <- game_data$team1_xgot*xgot1*m_xgotr[1]
    game_data$team2_xgot <- game_data$team2_xgot*xgot2*m_xgotr[2]
    game_data$team1_xa <- game_data$team1_xa*xa1*m_xar[1]
    game_data$team2_xa <- game_data$team2_xa*xa2*m_xar[2]
    ...
    manager_df$approx_games <- manager_df$m_minutes_xg/990
